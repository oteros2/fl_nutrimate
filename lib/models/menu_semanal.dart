...
class MenuSemanal {
  List<MenuDiario> menusDiarios;
  String nombreMenuSemanal;

  MenuSemanal({required this.menusDiarios, required this.nombreMenuSemanal});

  Map<String, dynamic> toMap() {
    return {
      'nombreMenuSemanal': nombreMenuSemanal,
      'menu': menusDiarios.map((m) => m.toMap()).toList(),
    };
  }

  factory MenuSemanal.fromMap(Map<String, dynamic> map) {
    return MenuSemanal(
      nombreMenuSemanal: map['nombreMenuSemanal'],
      menusDiarios:
          (map['menu'] as List).map((m) => MenuDiario.fromMap(m)).toList(),
    );
  }
}

// Ejemplo de instancia válida de MenuSemanal
final menu = MenuSemanal(
  nombreMenuSemanal: "Menú Semanal 1",
  menusDiarios: List.generate(7, (index) {
    return MenuDiario(nombreMenuDiario: 'Menu Diario ${index}', recetas: [
      Recipe(
        name: "Calamar en su tinta",
        imageUrl:
